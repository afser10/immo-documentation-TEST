...
export class DocumentTransformer extends ModelTransformer {
    ...
    parseRelationshipField(input, field, treeContext) {
        ...
        if (!rawIdIsStringOrNumber && field.list) {
            ...
            throw new ModelError(`Invalid "${this.definition.getTypeName()}" input object: ${JSON.stringify(rawId, null, 4)?.replace?.(`    "${field.fieldName}": `, `-->"${field.fieldName}": `)}\n\nThe value at input path "${treeContext.path.join(`.`)}" is not a valid node id string.`);
        }
        return rawId;
    }
    /*
    ...
    toConnectDefinition() {
        ...
        sdl += `type ${definition.prefixedTypeName()} implements Node @dontInfer${this.definition.getAuthorizationDirectiveSDL()} {\n`;
    ...
    toConnectSingleFieldValue(input, field, treeContext) {
        if (field.isRootValue) {
            ...
            const typename = this.definition.prefixedTypeName();
            return {
                id,
                internal: {
                    type: typename,
    ...
    toConnectListFieldValue(modelFieldValue) {
    ...
    toInsertedDocument(input, { visitorState, path }) {
        ...
        inputCopy.internal.type = this.definition.prefixedTypeName();
        return inputCopy;
    }
