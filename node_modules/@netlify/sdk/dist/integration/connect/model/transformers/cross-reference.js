...
export class CrossReferenceTransfromer extends ModelTransformer {
    ...
    parseVisitedInputList(input, field, treeContext) {
        ...
        if (!isArray) {
            throw new ModelError(`Input data for field ${field.fieldName} is not an array but should be. Received ${typeof input}.\n\n${JSON.stringify(input, null, 2)}`);
        }
  ...
  refId: ID!
  modelName: String! ${this.proxyFrom(`value.modelName`)}
  connectorName: String! ${this.proxyFrom(`refSrcType`)}
  instanceID: String! ${this.proxyFrom(`refProjectId`)}`;
...
type ${typeName} @dontInfer {${sharedCrossReferenceFields}
  reference: ${typeName}Union ${this.proxyFrom("value.reference")} @link
}
...
type ${typeName} @dontInfer${definition.getAuthorizationDirectiveSDL()} {${sharedCrossReferenceFields}
  reference: ${referenceTypePrefix + definition.crossReferenceTo[0].modelName} ${this.proxyFrom(`value.reference`)} @link
}`;
            }
        }
    }
    ...
    toConnectListFieldValue(modelFieldValue, field, treeContext) {
        ...
        return modelFieldValue?.map?.((node, index) => {
            return field.model.ModelWalker.coerceModelValue(node, field, [...path, field.fieldName, `[${index}]`], visitorState);
        });
    }
    /*
    ...
    toCreateListFieldValue(modelFieldValue, field, treeContext) {
        ...
        const listItems = modelFieldValue?.map?.((node, index) => {
            const fields = field.model.ModelWalker.coerceModelValue(node, field, [...path, field.fieldName, `[${index}]`], visitorState);
            return fields;
        });
        return {
            type: `list`,
