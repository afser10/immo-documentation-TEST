...
export class UnionTransformer extends ModelTransformer {
    ...
    getUnionEditorProperties(field) {
        ...
        if (field.type.isMixedUnion && !field.list) {
            localDevWarnOnce(`mixed union ${def.getTypeName()}`, `Found mixed union type ${def.getTypeName()}. Netlify Create does not support mixed union types unless the field is a list type, non-list fields of this type will not show up in the Netlify Create UI`);
            return null;
        }
    ...
    parseVisitedInputList(input, field, treeContext) {
        ...
        if (!Array.isArray(input)) {
            throw new ModelError(`Input data for field "${treeContext.path.join(`.`)}.${field.fieldName}" is not an array but should be. \n\nReceived ${JSON.stringify(input, null, 2)}`);
        }
        return input.map((item, index) => this.parseVisitedInputSingle(item, field, {
            path: [...treeContext.path, field.fieldName, `[${index}]`],
    ...
    toConnectListFieldValue(input, field, treeContext) {
        return input?.map?.((node, index) => {
            ...
            return model.ModelWalker.coerceModelValue(node, field, [...treeContext.path, field.fieldName, `[${index}]`], treeContext.visitorState);
        });
    }
    /*
    ...
    toCreateListFieldValue(modelFieldValue, field, treeContext) {
        ...
        const listItems = modelFieldValue?.map?.((node, index) => {
            ...
            const val = this.toCreateSingleFieldValue(node, field, {
                path: [...path, field.fieldName, `[${index}]`],
                visitorState,
            });
    ...
    toCreateSingleFieldValue(input, field, treeContext) {
        ...
        const memberType = this.parseToMemberType(input, path);
        if (memberType.isNodeType) {
            const rawId = this.parseInputRawId(input, memberType, path);
        ...
        const val = memberType.ModelWalker.coerceModelValue(input, field, path, visitorState);
        return {
            type: `model`,
            modelName: memberType.getOriginalName(),
