...
const { readFileSync, writeFile, writeJSON, copyFile, ensureDir, existsSync } = fsExtra;
/**
...
export async function createDevelopmentContentEngine({ integration, path, engineOptions, }) {
    const engineConfig = getEngineConfigWithBundledPlugin({
        integration,
        path,
    });
    const hooksPath = join(path, `/.ntli/connector/content-engine-hooks.cjs`);
...
export const generateConnectPlugin = async (integrationWorker, config, opts) => {
    ...
    const packageJSONContents = await fsExtra.readJSON(resolve(opts.directory || process.cwd(), `package.json`));
...
`);
        ...
        await writeFile(resolve(connectorDir, `netlify-create.js`), replaceTemplateDataString(templateFile, templateData));
    }
