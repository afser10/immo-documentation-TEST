...
export const generateBuildEventHandler = async (integration, config, opts) => {
  ...
  `;
    ...
    await buildInternalFunctions({ integration, outDir, type: "function" });
    // build any internal edge functions
    await buildInternalFunctions({ integration, outDir, type: "edge" });
...
const bundleEdgeFunction = async (srcPath, outPath) => {
    const esbuildOptions = {
        entryPoints: [srcPath],
        bundle: true,
        format: "esm",
...
const buildInternalFunctions = async ({ integration, outDir, type, }) => {
            ...
            for (const fn of internalFunctions || []) {
                ...
                try {
                    ...
                    if (stats.isFile()) {
                        ...
                        await bundleFunction(oldFilePath, newFilePath);
                    }
                    else if (stats.isDirectory()) {
                        await copyFunctionDirectory(oldFilePath, newFilePath, type === "function" ? bundleFunction : bundleEdgeFunction);
                    }
                }
                catch (err) {
                    console.error(err);
...
const bundleFunction = async (srcPath, outPath) => {
    const esbuildOptions = {
        entryPoints: [srcPath],
        bundle: true,
        format: "esm",
