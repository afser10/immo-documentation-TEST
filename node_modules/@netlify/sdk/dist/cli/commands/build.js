...
import { buildIntegrationUI, generateBuildEventHandler, generateConnectPlugin, processHandlers, processWrappers, } from "../index.js";
...
export const build = async (opts) => {
    const pwd = opts.directory || process.cwd();
    const isPreview = !!opts.previewUrl;
    const config = await getConfiguration(pwd);
    // remove dist folders
    ...
    if (existsSync(resolve(pwd, "tsconfig.json"))) {
        ...
        const tsconfig = `
      {
        ...
        "include": ["${resolve(pwd, integrationPackageJson.main)}"]
      }
    `;
        mkdirSync(resolve(pwd, ".ntli"));
        writeFileSync(resolve(pwd, ".ntli/tsconfig.build.json"), tsconfig);
        try {
    ...
    const integration = await getIntegrationModule(builtPath, opts.directory);
    await integrationWorker.loadModule(opts.directory);
    if (!integration) {
        return false;
    }
    if (all || buildtime) {
