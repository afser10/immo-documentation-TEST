...
var _lodash = _interopRequireDefault(require("lodash.topairs"));
var _lodash2 = _interopRequireDefault(require("lodash.difference"));
var _lodash3 = _interopRequireDefault(require("lodash.intersection"));
var _lodash4 = _interopRequireDefault(require("lodash.get"));
...
} = JSON.parse(_nodeFs.default.readFileSync(_path.default.join(_pkgDir.default.sync(__dirname), "package.json"), "utf8"));
...
function getBadExports(plugin, pluginAPIKeys, apis) {
  ...
  badExports = badExports.concat((0, _lodash2.default)(pluginAPIKeys, apis).map(e => {
    return {
      exportName: e,
...
async function handleBadExports({
  ...
  if (hasBadExports) {
    const latestAPIs = await (0, _getLatestApis.getLatestAPIs)();
    // Output error messages for all bad exports
    (0, _lodash.default)(badExports).forEach(badItem => {
...
async function validatePluginsOptions(plugins, rootDir) {
  ...
  const newPlugins = await Promise.all(plugins.map(async plugin => {
    if (
    // @ts-ignore
    plugin === `default-site-plugin` || plugin.resolve === `default-site-plugin`) {
      return plugin;
    }
    let gatsbyNode;
    try {
    ...
    let optionsSchema = gatsbyNode.pluginOptionsSchema({
      Joi: _pluginUtils.Joi.extend(joi => {
        return {
          type: `subPlugins`,
          base: joi.array().items(joi.alternatives(joi.string(), joi.object({
            resolve: _pluginUtils.Joi.string(),
            options: _pluginUtils.Joi.object({}).unknown(true)
    ...
    if (!_pluginUtils.Joi.isSchema(optionsSchema) || optionsSchema.type !== `object`) {
    ...
    try {
      ...
      if (!optionsSchema.describe().keys.plugins) {
        ...
        optionsSchema = optionsSchema.append({
          plugins: _pluginUtils.Joi.array().length(0)
        });
      }
      const {
        value,
        warning
      } = await (0, _pluginUtils.validateOptionsSchema)(optionsSchema, plugin.options || {});
      ...
      if (error instanceof _pluginUtils.Joi.ValidationError) {
...
async function collatePluginAPIs({
  ...
  for (const plugin of flattenedPlugins) {
    ...
    if (pluginNodeExports.length > 0) {
      plugin.nodeAPIs = (0, _lodash3.default)(pluginNodeExports, currentAPIs.node);
...
function warnOnIncompatiblePeerDependency(name, packageJSON) {
  ...
  const gatsbyPeerDependency = (0, _lodash4.default)(packageJSON, ["peerDependencies", "@netlify/content-engine"]);
  if (gatsbyPeerDependency && !semver.satisfies(gatsbyVersion, gatsbyPeerDependency, {
    includePrerelease: true
  })) {
    _reporter.default.warn(`Plugin ${name} is not compatible with your gatsby version ${gatsbyVersion} - It requires @netlify/content-engine@${gatsbyPeerDependency}`);
  }
}
