...
var _lodash = _interopRequireDefault(require("lodash.uniqwith"));
...
const resolveTheme = async (themeSpec, configFileThatDeclaredTheme, isMainConfig = false, rootDir) => {
  ...
  let themeDir;
  try {
    const scopedRequire = (0, _createRequireFromPath.createRequireFromPath)(`${rootDir}/:internal:`);
  ...
  const themeConfig = typeof theme === `function` ? theme(typeof themeSpec === `string` ? {} : themeSpec.options) : theme;
  return {
    themeName,
    themeConfig,
    themeSpec,
...
const processTheme = async ({
  ...
  const themesList = themeConfig === null || themeConfig === void 0 ? void 0 : themeConfig.plugins;
  ...
  if (themeConfig && themesList) {
    ...
    for (const spec of themesList) {
      ...
      result.push(await processTheme(themeObj, {
        rootDir: themeDir
      }));
    }
...
async function loadThemes(config, {
  ...
  const themesA = [];
  for (const plugin of (config === null || config === void 0 ? void 0 : config.plugins) || []) {
    if (typeof plugin !== `string` && (plugin === null || plugin === void 0 ? void 0 : plugin.resolve) === `default-site-plugin` || plugin === `default-site-plugin`) {
      continue;
    }
    const themeObj = await resolveTheme(plugin, configFilePath, true, rootDir);
    themesA.push(await processTheme(themeObj, {
      rootDir
    }));
  }
  ...
  for (const {
     ...
     */
    newConfig = (0, _mergeGatsbyConfig.mergeGatsbyConfig)({
      ...themeConfig,
  ...
  mergedConfig.plugins = (0, _lodash.default)(mergedConfig.plugins, _lodash2.default);
  return {
    config: mergedConfig,
    themes: themesAFlat
  };
