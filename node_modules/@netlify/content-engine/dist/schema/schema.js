...
var _typeDefs = require("./types/type-defs");
...
const freezeTypeComposers = (schemaComposer, excluded = new Set()) => {
  Array.from(schemaComposer.values()).forEach(tc => {
    const isCompositeTC = tc instanceof _graphqlCompose.ObjectTypeComposer || tc instanceof _graphqlCompose.InterfaceTypeComposer;
...
const updateSchemaComposer = async ({
  ...
  let activity = _reporter.default.phantomActivity(`Add explicit types`, {
  ...
  activity.end();
  activity = _reporter.default.phantomActivity(`Add inferred types`, {
    parentSpan: parentSpan
  });
  activity.start();
  await (0, _infer.addInferredTypes)({
    schemaComposer,
    typeConflictReporter,
    typeMapping,
    inferenceMetadata,
  ...
  activity.end();
  activity = _reporter.default.phantomActivity(`Processing types`, {
    parentSpan: parentSpan
  });
  activity.start();
  if (!process.env.GATSBY_SKIP_WRITING_SCHEMA_TO_FILE) {
    await (0, _print.printTypeDefinitions)({
      config: printConfig,
      schemaComposer
      // parentSpan: activity.span,
    });
...
const processTypeComposer = async ({
  ...
  if (typeComposer instanceof _graphqlCompose.ObjectTypeComposer) {
    await (0, _extensions.processFieldExtensions)({
      schemaComposer,
      typeComposer,
      fieldExtensions,
      parentSpan
    ...
    if (typeComposer.hasInterface(`Node`)) {
      await (0, _nodeInterface.addNodeInterfaceFields)({
        schemaComposer,
        typeComposer
      });
    }
    if (typeComposer.hasInterface(`RemoteFile`)) {
      (0, _remoteFileInterface.addRemoteFileInterfaceFields)(schemaComposer, typeComposer);
    }
  ...
  } else if (typeComposer instanceof _graphqlCompose.InterfaceTypeComposer) {
...
const fieldNames = {
  query: typeName => (0, _lodash.default)(typeName),
  queryAll: typeName => (0, _lodash.default)(`all ${typeName}`),
  convenienceChild: typeName => (0, _lodash.default)(`child ${typeName}`),
  convenienceChildren: typeName => (0, _lodash.default)(`children ${typeName}`)
};
const addTypes = ({
  schemaComposer,
  types,
  ...
  types.forEach(({
    ...
    if (typeof typeOrTypeDef === `string`) {
      typeOrTypeDef = (0, _typeDefs.parseTypeDef)(typeOrTypeDef);
    }
    if ((0, _typeDefs.isASTDocument)(typeOrTypeDef)) {
      let parsedTypes;
      ...
      try {
        ...
        (0, _typeDefs.reportParsingError)(error);
        return;
      }
...
const mergeTypes = ({
  ...
  const isSafeMerge = !plugin || plugin.name === `default-site-plugin` || plugin.name === typeOwner;
  if (!isSafeMerge) {
    if (typeOwner) {
      _reporter.default.warn(`Plugin \`${plugin.name}\` has customized the GraphQL type ` + `\`${typeComposer.getTypeName()}\`, which has already been defined ` + `by the plugin \`${typeOwner}\`. ` + `This could potentially cause conflicts.`);
    } else {
      _reporter.default.warn(`Plugin \`${plugin.name}\` has customized the built-in Gatsby GraphQL type ` + `\`${typeComposer.getTypeName()}\`. ` + `This is allowed, but could potentially cause conflicts.`);
    }
  }
  if (type instanceof _graphqlCompose.ObjectTypeComposer || type instanceof _graphqlCompose.InterfaceTypeComposer || type instanceof _graphql.GraphQLObjectType || type instanceof _graphql.GraphQLInterfaceType) {
    mergeFields({
      typeComposer,
  ...
  if (type instanceof _graphql.GraphQLInterfaceType || type instanceof _graphqlCompose.InterfaceTypeComposer || type instanceof _graphql.GraphQLUnionType || type instanceof _graphqlCompose.UnionTypeComposer) {
    mergeResolveType({
      typeComposer,
      type
    });
...
const processAddedType = ({
  ...
  const typeComposer = schemaComposer.get(typeName);
  if (typeComposer instanceof _graphqlCompose.InterfaceTypeComposer || typeComposer instanceof _graphqlCompose.UnionTypeComposer) {
    if (!typeComposer.getResolveType()) {
      typeComposer.setResolveType(node => {
        var _node$internal;
        return (node === null || node === void 0 ? void 0 : (_node$internal = node.internal) === null || _node$internal === void 0 ? void 0 : _node$internal.type) || (node === null || node === void 0 ? void 0 : node.__typename);
      });
    }
  }
  ...
  directives.forEach(({
    ...
    switch (name) {
      ...
      case `childOf`:
        ...
        break;
      case `nodeInterface`:
        if (typeComposer instanceof _graphqlCompose.InterfaceTypeComposer) {
...
const addExtensions = ({
  ...
  if (typeComposer instanceof _graphqlCompose.InterfaceTypeComposer && isNodeInterface(typeComposer)) {
    const hasCorrectIdField = typeComposer.hasField(`id`) && typeComposer.getFieldType(`id`).toString() === `ID!`;
    if (!hasCorrectIdField) {
      _reporter.default.panic(`Interfaces with the \`nodeInterface\` extension must have a field ` + `\`id\` of type \`ID!\`. Check the type definition of ` + `\`${typeComposer.getTypeName()}\`.`);
    }
  }
  if (typeComposer instanceof _graphqlCompose.ObjectTypeComposer || typeComposer instanceof _graphqlCompose.InterfaceTypeComposer || typeComposer instanceof _graphqlCompose.InputTypeComposer) {
    typeComposer.getFieldNames().forEach(fieldName => {
      ...
      Object.keys(fieldExtensions).filter(name => !_extensions.internalExtensionNames.includes(name)).forEach(name => {
        const args = fieldExtensions[name];
        if (!args || typeof args !== `object`) {
          _reporter.default.error(`Field extension arguments must be provided as an object. ` + `Received "${args}" on \`${typeName}.${fieldName}\`.`);
          return;
        }
        try {
          ...
          Object.keys(args).forEach(arg => {
            ...
            if (!argumentDef) {
              _reporter.default.error(`Field extension \`${name}\` on \`${typeName}.${fieldName}\` ` + `has invalid argument \`${arg}\`.`);
              return;
            }
            ...
            try {
              ...
              _reporter.default.error(`Field extension \`${name}\` on \`${typeName}.${fieldName}\` ` + `has argument \`${arg}\` with invalid value "${value}". ` + error.message);
            }
          });
        } catch (error) {
          _reporter.default.error(`Field extension \`${name}\` on \`${typeName}.${fieldName}\` ` + `is not available.`);
        }
      });
    });
  }
...
const checkIsAllowedTypeName = name => {
  (0, _invariant.default)(name !== `Node`, `The GraphQL type \`Node\` is reserved for internal use.`);
  (0, _invariant.default)(!name.endsWith(`FilterInput`) && !name.endsWith(`SortInput`), `GraphQL type names ending with "FilterInput" or "SortInput" are ` + `reserved for internal use. Please rename \`${name}\`.`);
  (0, _invariant.default)(!_builtInTypes.builtInScalarTypeNames.includes(name), `The GraphQL type \`${name}\` is reserved for internal use by ` + `built-in scalar types.`);
...
const createTypeComposerFromGatsbyType = ({
  ...
  switch (type.kind) {
    case _typeBuilders.GatsbyGraphQLTypeKind.OBJECT:
      {
        typeComposer = _graphqlCompose.ObjectTypeComposer.createTemp({
          ...type.config,
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.INPUT_OBJECT:
      {
        typeComposer = _graphqlCompose.InputTypeComposer.createTemp({
          ...type.config,
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.UNION:
      {
        typeComposer = _graphqlCompose.UnionTypeComposer.createTemp({
          ...type.config,
          types: () => {
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.INTERFACE:
      {
        typeComposer = _graphqlCompose.InterfaceTypeComposer.createTemp({
          ...type.config,
          fields: () => schemaComposer.typeMapper.convertOutputFieldConfigMap(type.config.fields),
          interfaces: () => {
            if (type.config.interfaces) {
    ...
    case _typeBuilders.GatsbyGraphQLTypeKind.ENUM:
      {
        typeComposer = _graphqlCompose.EnumTypeComposer.createTemp(type.config);
        break;
      }
    case _typeBuilders.GatsbyGraphQLTypeKind.SCALAR:
      {
        typeComposer = _graphqlCompose.ScalarTypeComposer.createTemp(type.config);
        break;
      }
    default:
      {
        _reporter.default.warn(`Illegal type definition: ${JSON.stringify(type.config)}`);
        typeComposer = null;
      }
  }
  if (typeComposer) {
...
const addSetFieldsOnGraphQLNodeTypeFields = ({
  ...
  if (tc instanceof _graphqlCompose.ObjectTypeComposer && tc.hasInterface(`Node`)) {
    const typeName = tc.getTypeName();
    const result = await (0, _apiRunnerNode.default)(`setFieldsOnGraphQLNodeType`, {
      type: {
        name: typeName,
        get nodes() {
          // TODO STRICT_MODE: return iterator instead of array
          return (0, _datastore.getNodesByType)(typeName);
        }
      },
...
const addThirdPartySchemas = ({
  ...
  thirdPartySchemas.forEach(schema => {
    ...
    Object.keys(types).forEach(typeName => {
      ...
      if (type !== schemaQueryType && !(0, _graphql.isSpecifiedScalarType)(type) && !(0, _graphql.isIntrospectionType)(type) && type.name !== `Date` && type.name !== `JSON`) {
        const typeHasFields = type instanceof _graphql.GraphQLObjectType || type instanceof _graphql.GraphQLInterfaceType;
  ...
  const createResolvers = (resolvers, {
    ...
    Object.keys(resolvers).forEach(typeName => {
      ...
      if (schemaComposer.has(typeName)) {
        ...
        Object.keys(fields).forEach(fieldName => {
          ...
          if (tc.hasField(fieldName)) {
            ...
            if (!fieldTypeName || fieldTypeName.replace(/!/g, ``) === originalTypeName.replace(/!/g, ``) || tc.getExtension(`createdFrom`) === `thirdPartySchema`) {
              ...
              _reporter.default.warn(`\`createResolvers\` passed resolvers for field ` + `\`${typeName}.${fieldName}\` with type \`${fieldTypeName}\`. ` + `Such a field with type \`${originalTypeName}\` already exists ` + `on the type. Use \`createTypes\` to override type fields.`);
            }
          } else {
            tc.addFields({
        ...
        _reporter.default.warn(`\`createResolvers\` passed resolvers for type \`${typeName}\` that ` + `doesn't exist in the schema. Use \`createTypes\` to add the type ` + `before adding resolvers.`);
      }
    });
  };
  await (0, _apiRunnerNode.default)(`createResolvers`, {
    intermediateSchema,
    createResolvers,
  ...
  schemaComposer.forEach(typeComposer => {
    if (typeComposer instanceof _graphqlCompose.ObjectTypeComposer || typeComposer instanceof _graphqlCompose.InterfaceTypeComposer) {
      typeComposer.getFieldNames().forEach(fieldName => {
        ...
        const resolver = (0, _resolvers.wrappingResolver)(field.resolve || _resolvers.defaultResolver);
        typeComposer.extendField(fieldName, {
          resolve: resolver
        });
      });
...
const determineSearchableFields = ({
  ...
  typeComposer.getFieldNames().forEach(fieldName => {
    ...
    if (field.resolve) {
      ...
      } else if (!(0, _lodash2.default)(field.args)) {
        typeComposer.extendFieldExtensions(fieldName, {
...
const addConvenienceChildrenFields = ({
  ...
  schemaComposer.forEach(type => {
    if ((type instanceof _graphqlCompose.ObjectTypeComposer || type instanceof _graphqlCompose.InterfaceTypeComposer) && type.hasExtension(`mimeTypes`)) {
      // @ts-ignore
      const {
        types
    ...
    if ((type instanceof _graphqlCompose.ObjectTypeComposer || type instanceof _graphqlCompose.InterfaceTypeComposer) && type.hasExtension(`childOf`)) {
      if (type instanceof _graphqlCompose.ObjectTypeComposer && !type.hasInterface(`Node`)) {
        _reporter.default.error(`The \`childOf\` extension can only be used on types that implement the \`Node\` interface.\n` + `Check the type definition of \`${type.getTypeName()}\`.`);
        return;
      }
      if (type instanceof _graphqlCompose.InterfaceTypeComposer && !isNodeInterface(type)) {
        _reporter.default.error(`The \`childOf\` extension can only be used on types that implement the \`Node\` interface.\n` + `Check the type definition of \`${type.getTypeName()}\`.`);
        return;
      }
  ...
  parentTypesToChildren.forEach((children, parent) => {
    ...
    if (typeComposer instanceof _graphqlCompose.InterfaceTypeComposer && !isNodeInterface(typeComposer)) {
      _reporter.default.error(`With the \`childOf\` extension, children fields can only be added to ` + `interfaces which implement the \`Node\` interface.\n` + `Check the type definition of \`${typeComposer.getTypeName()}\`.`);
      return;
    }
...
const addInferredChildOfExtensions = ({
  ...
  schemaComposer.forEach(typeComposer => {
    if (typeComposer instanceof _graphqlCompose.ObjectTypeComposer && typeComposer.hasInterface(`Node`)) {
...
const addInferredChildOfExtension = ({
  ...
  for (const node of (0, _datastore.getDataStore)().iterateNodesByType(parentTypeName)) {
    const children = (node.children || []).map(_datastore.getNode);
...
const addTypeToRootQuery = ({
  ...
  schemaComposer.Query.addFields({
    [queryName]: {
      ...
      resolve: (0, _resolvers.findOne)(typeName)
    },
    [queryNamePlural]: {
      type: paginationTC,
      args: {
        filter: filterInputTC,
        sort: (0, _sort.getSortInputNestedObjects)({
          schemaComposer,
          typeComposer
        }),
        skip: `Int`,
      ...
      resolve: (0, _resolvers.findManyPaginated)(typeName)
    }
...
const isNamedTypeComposer = type => type instanceof _graphqlCompose.ObjectTypeComposer || type instanceof _graphqlCompose.InputTypeComposer || type instanceof _graphqlCompose.ScalarTypeComposer || type instanceof _graphqlCompose.EnumTypeComposer || type instanceof _graphqlCompose.InterfaceTypeComposer || type instanceof _graphqlCompose.UnionTypeComposer;
...
const checkQueryableInterfaces = ({
  ...
  schemaComposer.forEach(type => {
    if (type instanceof _graphqlCompose.ObjectTypeComposer) {
  ...
  if (incorrectTypes.size) {
    _reporter.default.panic(`Types implementing queryable interfaces must also implement the \`Node\` ` + `interface. Check the type definition of ` + `${Array.from(incorrectTypes).map(t => `\`${t}\``).join(`, `)}.`);
  }
};
const mergeFields = ({
  typeComposer,
...
const mergeResolveType = ({
  ...
  if ((type instanceof _graphql.GraphQLInterfaceType || type instanceof _graphql.GraphQLUnionType) && type.resolveType) {
    typeComposer.setResolveType(type.resolveType);
  }
  if ((type instanceof _graphqlCompose.InterfaceTypeComposer || type instanceof _graphqlCompose.UnionTypeComposer) && type.getResolveType()) {
    typeComposer.setResolveType(type.getResolveType());
  }
  if (!typeComposer.getResolveType()) {
    typeComposer.setResolveType(node => {
      var _node$internal2;
      return node === null || node === void 0 ? void 0 : (_node$internal2 = node.internal) === null || _node$internal2 === void 0 ? void 0 : _node$internal2.type;
    });
  }
};
