...
var _isObject = require("../core-utils/is-object");
...
class LocalNodeModel {
  ...
  getNodesByIds(args) {
    ...
    const nodes = Array.isArray(ids) ? ids.map(id => {
      if (typeof id === `string`) return getNodeById(id);
      if (keepObjects) {
        return id;
      } else {
        return null;
    ...
    if (!nodes.length || !type) {
      ...
      const nodeTypeNames = (keepObjects ? _utils.toTypeNames : _utils.toNodeTypeNames)(
      // @ts-ignore
      this.schema, type);
      ...
      result = nodes.filter(node => {
        var _node$internal;
        return node !== null && node !== void 0 && (_node$internal = node.internal) !== null && _node$internal !== void 0 && _node$internal.type ? nodeTypeNames.includes(node.internal.type) : nodeTypeNames.includes(node === null || node === void 0 ? void 0 : node.__typename);
      });
    }
    return result;
  }
  async _query(args) {
    ...
    (0, _invariant.default)(!(gqlType instanceof _graphql.GraphQLUnionType), `Querying GraphQLUnion types is not supported.`);
    ...
    if (typeof ((_query = query) === null || _query === void 0 ? void 0 : (_query$filter = _query.filter) === null || _query$filter === void 0 ? void 0 : (_query$filter$id = _query$filter.id) === null || _query$filter$id === void 0 ? void 0 : _query$filter$id.eq) !== `undefined` && Object.keys(query.filter).length === 1 && Object.keys(query.filter.id).length === 1) {
      if (tracer) {
        runQueryActivity = _reporter.default.phantomActivity(`runQuerySimpleIdEq`, {
    ...
    if (tracer) {
      materializationActivity = _reporter.default.phantomActivity(`Materialization`, {
        parentSpan: tracer.getParentActivity().span
      });
    ...
    if (tracer) {
      runQueryActivity = _reporter.default.phantomActivity(`runQuery`, {
        parentSpan: tracer.getParentActivity().span
      });
      runQueryActivity.start();
    }
  ...
  async _doResolvePrepareNodesQueue(type) {
    ...
    const {
      ...
      return {
        queryFields: (0, _lodash4.default)(queryFields, nextQueryFields),
        fieldsToResolve: (0, _lodash4.default)(fieldsToResolve, nextFieldsToResolve)
      };
    }, {
      queryFields: {},
      fieldsToResolve: {}
    ...
    if (!(0, _lodash.default)(actualFieldsToResolve)) {
      const {
        schemaCustomization: {
      ...
      // @ts-ignore
      this._preparedNodesCache.set(typeName, (0, _lodash4.default)({},
      // @ts-ignore
        ...
        if (tracked) {
          ...
          while (iterations++ < 100) {
            ...
            if (visited.has(tracked)) {
              _reporter.default.error(`It looks like you have a node that's set its parent as itself:\n\n` + tracked);
              break;
            }
            visited.add(tracked);
...
const getQueryFields = ({
  ...
  return (0, _lodash4.default)(filterFields, ...sortFields.map(pathToObject), ...group.map(pathToObject), ...distinct.map(pathToObject), ...max.map(pathToObject), ...min.map(pathToObject), ...sum.map(pathToObject));
};
...
const dropQueryOperators = filter => Object.keys(filter).reduce((acc, key) => {
  ...
  const v = value[k];
  if ((0, _lodash3.default)(value) && (0, _lodash3.default)(v)) {
...
async function resolveRecursive(nodeModel, schemaComposer, schema, node, type, queryFields, fieldsToResolve, customContext) {
  ...
  for (const fieldName of Object.keys(fieldsToResolve)) {
    ...
    const gqlNonNullType = (0, _graphql.getNullableType)(gqlField.type);
    const gqlFieldType = (0, _graphql.getNamedType)(gqlField.type);
    ...
    if (gqlField && innerValue != null) {
      if ((0, _graphql.isCompositeType)(gqlFieldType) && !(gqlNonNullType instanceof _graphql.GraphQLList)) {
        innerValue = await resolveRecursive(nodeModel, schemaComposer, schema, innerValue, gqlFieldType, queryField, (0, _isObject.isObject)(fieldToResolve) ? fieldToResolve : queryField, customContext);
      } else if ((0, _graphql.isCompositeType)(gqlFieldType) && (Array.isArray(innerValue) || innerValue instanceof _iterable.GatsbyIterable) && gqlNonNullType instanceof _graphql.GraphQLList) {
        innerValue = await Promise.all(innerValue.map(item => item == null ? item : resolveRecursive(nodeModel, schemaComposer, schema, item, gqlFieldType, queryField, (0, _isObject.isObject)(fieldToResolve) ? fieldToResolve : queryField, customContext)));
      }
    }
    if (innerValue != null) {
  ...
  // @ts-ignore
  return (0, _lodash2.default)(resolvedFields, (value, key) => queryFields[key]);
}
...
function resolveField(nodeModel, schemaComposer, schema, node, gqlField, fieldName, customContext) {
  ...
  if (!withResolverContext) {
    withResolverContext = (0, _preferDefault.preferDefault)(require(`./context`));
  }
...
const determineResolvableFields = (schemaComposer, schema, type, fields, isNestedAndParentNeedsResolve = false) => {
  ...
  Object.keys(fields).forEach(fieldName => {
    ...
    if ((0, _isObject.isObject)(field) && gqlField) {
      ...
      if (!(0, _lodash.default)(innerResolved)) {
...
const deepObjectDifference = (from, to) => {
  ...
  Object.keys(from).forEach(key => {
    ...
    if (toValue) {
      if ((0, _lodash3.default)(toValue)) {
        const deepResult = deepObjectDifference(from[key], toValue);
        if (!(0, _lodash.default)(deepResult)) {
