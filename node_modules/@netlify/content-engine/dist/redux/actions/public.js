...
const findChildren = initialChildren => {
  ...
  while (queue.length > 0) {
    const currentChild = (0, _datastore.getNode)(queue.pop());
...
actions.deleteNode = (node, plugin) => {
  ...
  const internalNode = (0, _datastore.getNode)(id);
  ...
  const deleteDescendantsActions = internalNode && findChildren(internalNode.children).map(_datastore.getNode).map(createDeleteAction);
...
function getNextNodeCounter() {
  ...
  const lastNodeCounter = (_store$getState$statu = _index.store.getState().status.LAST_NODE_COUNTER) !== null && _store$getState$statu !== void 0 ? _store$getState$statu : 0;
 ...
 * })
 */
const createNode = (node, plugin,
// @ts-ignore
actionOptions = {}) => {
  if (!(0, _isObject.isObject)(node)) {
    return console.log(_chalk.default.bold.red(`The node passed to the "createNode" action creator must be an object`));
  }
  ...
  if (node.internal.owner) {
    _reporter.default.error(JSON.stringify(node, null, 4));
    _reporter.default.panic(_chalk.default.bold.red(`The node internal.owner field is set automatically by Gatsby and not by plugins`));
  }
  const trackParams = {};
  ...
  const oldNode = (0, _datastore.getNode)(node.id);
  ...
  if (oldNode && !(0, _nodes.hasNodeChanged)(node.id, node.internal.contentDigest)) {
    updateNodeAction = {
      ...actionOptions,
      plugin,
      type: `TOUCH_NODE`,
    ...
    if (oldNode) {
      ...
      deleteActions = findChildren(oldNode.children).map(_datastore.getNode).map(createDeleteAction);
    }
...
actions.createNode = (...args) => dispatch => {
  ...
  const maybePromise = (0, _apiRunnerNode.default)(`onCreateNode`, {
  ...
  if (maybePromise !== null && maybePromise !== void 0 && maybePromise.then) {
    return maybePromise.then(res => (0, _datastore.getDataStore)().ready().then(() => res));
  } else {
    return (0, _datastore.getDataStore)().ready().then(() => maybePromise);
  }
};
...
actions.createNodeField = ({
  ...
  const schemaFieldName = name !== null && name !== void 0 && name.includes(`___NODE`) ?
  // @ts-ignore
  name.split(`___`)[0] : name;
...
actions.setBabelOptions = (options, plugin) => {
  ...
  // @ts-ignore
  plugin.name}"`;
  if ((plugin === null || plugin === void 0 ? void 0 : plugin.name) === `default-site-plugin`) {
    name = `Your site's "gatsby-node.js"`;
  }
  if (!(0, _isObject.isObject)(options)) {
  ...
  // @ts-ignore
  if (!(0, _isObject.isObject)(options.options)) {
...
actions.setBabelPlugin = (config, plugin) => {
  // Validate
  let name = `The plugin "${(plugin === null || plugin === void 0 ? void 0 : plugin.name) || `unknown`}"`;
  if ((plugin === null || plugin === void 0 ? void 0 : plugin.name) === `default-site-plugin`) {
    name = `Your site's "gatsby-node.js"`;
  }
  // @ts-ignore
...
actions.createJobV2 = (job, plugin) => (dispatch, getState) => {
  // @ts-ignore
  const internalJob = (0, _manager.createInternalJob)(job, plugin);
...
actions.setRequestHeaders = ({
  ...
  if (noHeaders) {
    _reporter.default.warn(`Plugin ${plugin.name} called actions.setRequestHeaders with a headers property that isn't an object.`);
  }
  if (noDomain) {
    _reporter.default.warn(`Plugin ${plugin.name} called actions.setRequestHeaders with a domain property that isn't a string.`);
  }
  if (noDomain || noHeaders) {
    _reporter.default.panic(`Plugin ${plugin.name} attempted to set request headers with invalid arguments. See above warnings for more info.`);
    return null;
  }
  const baseDomain = (_url$parse = _url.default.parse(domain)) === null || _url$parse === void 0 ? void 0 : _url$parse.hostname;
  if (baseDomain) {
    return {
    ...
    _reporter.default.panic(`Plugin ${plugin.name} attempted to set request headers for a domain that is not a valid URL. (${domain})`);
    return null;
  }
};
